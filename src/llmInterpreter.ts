...
export function parseLLMJson(raw: string): LLMDirectiveResponse {
  ...
  if (
    ...
    parsed === null ||
    !Array.isArray(
      (parsed as { directive_interpretation?: unknown }).directive_interpretation
    )
  ) {
...
async function callGemini(
  ...
  const text =
    ...
    "";
  if (!text) {
    console.error(`[gemini:${model}] empty response body:`, JSON.stringify(json).slice(0, 300));
...
async function callOpenAI(
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    console.error(`[openai] HTTP ${res.status}: ${text.slice(0, 500)}`);
...
async function callLLMWithFallback(
  ...
  // Try each key with each model in order.
  for (const { name, key } of geminiKeys) {
    if (!key) {
      console.error(`[fallback] ${name} not configured`);
      continue;
    }
  ...
  if (env.OPENAI_API_KEY) {
    try {
      ...
      errors.push(`OPENAI/${OPENAI_MODEL}: ${msg}`);
    }
  } else {
...
// ============================================================================
// Retry message from guardrail failures
// ============================================================================
