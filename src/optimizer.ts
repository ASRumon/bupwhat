...
//
// Pipeline:
//   Step 1   effective_solar (solar_reduction directives)
//   Step 2   constraint overlays (grid_cap, no_charge, no_discharge, floor)
//   Step 2.5 backward reachability pass
//   Step 3   forward greedy construction
//   Step 3.5 charge-creation pass (pair cheap-hour charge with expensive-hour discharge)
//   Step 4   end-of-day neutrality (cost-optimized explicit moves)
//   Step 5   local improvement (explicit 2-opt shift)
//   Step 6   hard invariant enforcement at every step
// ============================================================================
...
const MAX_CREATE_ITERATIONS = 5000;
...
// ---------------------------------------------------------------------------
// Step 1 + 2 — Build per-hour overlays from request + directives
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Step 2.5 — Backward reachability
// ---------------------------------------------------------------------------
...
//
// NOTE: The discharge guard has been relaxed vs. the original version.
// The backward pass already computed effective_min_energy to encode every
// downstream obligation, so requiring "room for a full max_discharge" is
// overly conservative and causes the battery to idle when E is near
// effective_min_energy + max_discharge. We now discharge up to the safe
// headroom E - effective_min_energy[h], bounded by max_discharge and the
// hour's remaining net demand.
// ---------------------------------------------------------------------------
function greedyForward(
  ...
  for (let h = 0; h < 24; h++) {
    ...
    if (net >= 0) {
      ...
      // FIX 1 — discharge as much as is SAFE given the backward-pass floor.
      if (!internal[h].no_discharge) {
        const headroom = Math.max(0, E - effective_min_energy[h]);
        dis = Math.min(max_discharge, remaining, headroom);
      }
...
// ---------------------------------------------------------------------------
// Invariant checker (full 24-hour replay)
// ---------------------------------------------------------------------------
function checkInvariants(
  ...
  for (let h = 0; h < 24; h++) {
    ...
    if (Math.abs(bal) > EPS)
     ...
     if (state.solar_used[h] > internal[h].effective_solar + EPS)
   errors.push(
     `h${h}: solar_used ${state.solar_used[h]} > effective_solar ${internal[h].effective_solar}`
   );
...
// ---------------------------------------------------------------------------
// Step 3.5 — Charge creation (arbitrage the greedy missed)
//
// The forward greedy only charges from surplus solar. When the scenario has
// little or no solar surplus, the battery never charges, so the 2-opt pass
// has nothing to shift. This pass adds 1 kWh of charge to the CHEAPEST
// eligible deficit hour and simultaneously 1 kWh of discharge to the MOST
// EXPENSIVE eligible deficit hour. Both move together, so end-of-day
// neutrality is preserved by construction. Accept a pair only if the full
// 24-hour replay passes all invariants and the cost strictly improves.
// ---------------------------------------------------------------------------
function createCharge(
  ...
  while (changed && iter < MAX_CREATE_ITERATIONS) {
    iter++;
    changed = false;

    // Cheapest eligible deficit hour for +1 kWh charge.
    const chargeCands: number[] = [];
    for (let h = 0; h < 24; h++) {
      if (internal[h].no_charge) continue;
      if (internal[h].demand < internal[h].effective_solar) continue; // surplus
      if (state.charge[h] + 1 > max_charge + EPS) continue;
      const E_before_h =
        h === 0 ? battery.initial_energy_kwh : state.E_after[h - 1];
      if (E_before_h + 1 > capacity + EPS) continue;
      // Adding 1 kWh of charge at a deficit hour increases grid by 1 kWh.
      if (
        Number.isFinite(internal[h].grid_cap) &&
    ...
    chargeCands.sort((a, b) => internal[a].tariff - internal[b].tariff);

    if (chargeCands.length === 0) break;

    // Most expensive eligible deficit hour for +1 kWh discharge.
    const disCands: number[] = [];
    for (let h = 0; h < 24; h++) {
      if (internal[h].no_discharge) continue;
      if (internal[h].demand < internal[h].effective_solar) continue; // surplus
      if (state.discharge[h] + 1 > max_discharge + EPS) continue;
      disCands.push(h);
    }
    disCands.sort((a, b) => internal[b].tariff - internal[a].tariff);

    if (disCands.length === 0) break;

    const oldCost = computeCost(internal, state);

    let applied = false;
    for (const a of chargeCands) {
      for (const b of disCands) {
        // Require a positive tariff spread.
        if (internal[b].tariff - internal[a].tariff <= EPS) continue;
        // Charge and discharge must be at different hours.
        if (a === b) continue;
        ...
        // Neutrality is preserved by construction (both +1), so requireNeutral = true.
    ...
    if (delta > 0) {
      ...
      for (let h = 0; h < 24; h++) {
        ...
        if (
          ...
          cands.push({ hour: h, kind: "B", savings: internal[h].tariff });
        }
      }
...
// ---------------------------------------------------------------------------
// Step 5 — Local 2-opt improvement (shift charge / shift discharge)
// ---------------------------------------------------------------------------
function localImprovement(
  ...
  while (improved && iter < MAX_OPT_ITERATIONS) {
    ...
    // MOVE A: shift charge from hour a to hour b (a<b) if tariff[a] > tariff[b]
    ...
    // MOVE B: shift discharge from hour a to hour b (a<b) if tariff[a] < tariff[b]
...
export function optimize(
  ...
  const reqWithDirectives = { ...req, directives };
...
console.log
  ...
  // FIX 2 — create cheap-charge / expensive-discharge pairs the greedy missed.
  const charged = createCharge(
    internal,
    req.battery,
    backward.effective_min_energy,
    greedy
