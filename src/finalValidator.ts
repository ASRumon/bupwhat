...
export function replayValidate(
  ...
  for (let h = 0; h < 24; h++) {
    ...
    // --- FIX: use HourlySolution field names (grid, solar_used, battery_energy_after) ---
    if (p.solar_used > internal[h].effective_solar + EPS) {
      throw new ApiError(
        500,
        "validation_failed",
        `h${h}: solar_used ${p.solar_used} > effective_solar ${internal[h].effective_solar}`
      );
    }

    if (
      Number.isFinite(internal[h].grid_cap) &&
      p.grid > internal[h].grid_cap + EPS
    ) {
      throw new ApiError(
        500,
        "validation_failed",
        `h${h}: grid ${p.grid} > cap ${internal[h].grid_cap}`
      );
    }

    if (p.grid < -EPS || p.solar_used < -EPS) {
      throw new ApiError(
        500,
        "validation_failed",
    ...
    const lhs = p.grid + p.solar_used + dis;
    ...
    // --- FIX: use battery_energy_after ---
    if (Math.abs(E - p.battery_energy_after) > EPS) {
      throw new ApiError(
        500,
        "validation_failed",
        `h${h}: battery_energy_after mismatch stored=${p.battery_energy_after} computed=${E}`
      );
    }

    // --- FIX: use p.grid ---
    totalGrid += p.grid;
    totalCost += p.grid * internal[h].tariff;
    if (p.grid > peakGrid) peakGrid = p.grid;
  }
